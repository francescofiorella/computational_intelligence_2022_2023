...
class Quarto(object):
    ...
    def reset(self):
        ...
        self.__binary_board = np.full(shape=(self.BOARD_SIDE, self.BOARD_SIDE, 4), fill_value=np.nan)
        self.__pieces = []
    ...
    def place(self, x: int, y: int) -> bool:
        ...
        if self.__placeable(x, y):
            ...
            self.__binary_board[y, x][:] = self.__pieces[self.__selected_piece_index].binary
            return True
        return False
